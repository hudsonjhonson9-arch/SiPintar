...
function qrUrl(idAlat, size = 300) {
  const target = `${window.location.origin}/?id=${encodeURIComponent(idAlat)}`;
...
export default function TabAlat() {
  ...
  async function handleDownloadQR(item) {
    setDownloadingId(item.idAlat);
    setError("");
    try {
      const res = await fetch(qrUrl(item.idAlat, 500));
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `qr-${item.idAlat}.png`;
        ...
        {items && items.map((item) => (
          <div className="admin-row-card" key={item.idAlat}>
            <div className="admin-row-main">
              <img className="admin-qr-thumb" src={qrUrl(item.idAlat, 80)} alt={`QR ${item.idAlat}`} />
              <div style={{ flex: 1 }}>
            ...
            <div className="admin-row-actions">
              ...
              <button className="admin-link-btn" onClick={() => handleDownloadQR(item)} disabled={downloadingId === item.idAlat}>
                {downloadingId === item.idAlat ? "Mengunduh..." : "Unduh QR"}
              </button>
              {item.status === "DIPINJAM" && (
